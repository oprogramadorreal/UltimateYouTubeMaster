...
struct UCLAGL_v2g
{
	...
	float4  posWorld : TEXCOORD2; // vertex position in world space
};
...
float _PlayerPlanetRadius;

float calculateFragAlpha(UCLAGL_g2f input)
{
	float alpha = 0.0f;

	float maxPlayerDistance = 50.0f;
	float minPlayerDistance = 20.0f;
	float playerDistance = distance(input.posWorld, _PlayerPosition) - _PlayerPlanetRadius;
	...
	{
		alpha = 1.0f - playerDistance / (maxPlayerDistance - minPlayerDistance);
	}
